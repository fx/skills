...
**Where two or more channels carry blocking findings, fix them once, together, before dispatching any resolver.** Two resolvers editing the same branch in parallel race on the working tree and produce two pushes — two CI cycles and two CodeRabbit re-reviews for one round of feedback (`[SKILLS_DIR]/dev/references/head-discipline.md` § Batch findings). Instead: hand the whole blocking set to one fix agent, push once, then invoke each resolver with its blocking threads annotated `already fixed in <sha>` (`[SKILLS_DIR]/dev/references/scope-contract.md` § Resolver dispositions). The resolvers then do what only they can do — reply, resolve, and record `REVIEW.md` entries — with no further edits and no further pushes.
