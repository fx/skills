...
Where the diff changed **materially** since Step 4.5 converged — a Step 6.2 or Step 7.2 fix push that touched new files or new behaviour, not a one-line correction — re-enter **Step 4.5's Codex pass on the delta** before launching 6.3, and count that invocation against `references/scope-contract.md` § The iteration bound. The trigger is a material change to the diff, **not** a head move: a head that moved without changing what the reviewed code does needs no re-run, and there is no other local reviewer to reach for.

**This is the material-change test, and every path that re-covers the hosted reviewers on a new head applies it before the relaunch** — the Step 6 loop's item 6, 6.3's per-reviewer step 4, and Step 7.2. It is stated here and only here; those sites reference it. Applying it costs nothing at the common case, because the wording above already lets a push that did not change what the reviewed code does fall straight through.
