...
| Wait | Completion notification for a backgrounded `Agent` wakes the coordinator; `TaskOutput` for its report; `TaskList` for a status snapshot. Nothing to poll, and a live teammate survives the coordinator's turn ending |
