...
**A `blocking` disposition may arrive annotated `already fixed in <sha>`.** That is not a fourth disposition — it is the same one, with the fix already carried out. The coordinator batches blocking fixes from every channel into one commit and one push (`head-discipline.md` § Batch findings), so by the time a resolver sees the thread the code is already correct. Treat it as reply-and-resolve: cite the commit, resolve the thread, and **make no edit and no push**. Verify the fix is in the branch before resolving; if it is not, the annotation is wrong — report that rather than resolving on it. A resolver that re-fixes an already-fixed finding produces a redundant push, and every push costs a CI cycle and a re-review.
