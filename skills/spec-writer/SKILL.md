...
- **Reviews with no context.** Specs and change documents get reviewed like code, and a reviewer that does not know this is a docs-only change reports missing implementation, missing tests, and absent dependencies. **Every review of the output — `codex-review`, `copilot-review`, `coderabbit-review`, or a reviewer a user invokes by hand — MUST be governed by a Scope Brief stating that this change is documentation only, that the described work is planned rather than implemented, and which divergences from current code are deliberate.** The brief goes to the **invocation** in every case; how it reaches the reviewer behind it does not. `codex-review` passes it as the review prompt, so Codex genuinely receives it. Copilot and the CodeRabbit GitHub App accept no prompt at all — the brief cannot reach them, so `copilot-review` and `coderabbit-review` apply it when **triaging** what comes back. See `[SKILLS_DIR]/dev/references/scope-contract.md` § Injecting the brief into reviews for the per-reviewer mechanics; do not restate them here.

  The requirement is the same either way, and it is why this rule exists: **missing implementation, missing tests, and absent dependencies are never findings against a document describing work not yet done.** Whichever route the brief took, such a finding is recorded as deferred against the exclusion that covers it — never fixed by writing code, which § Scope — Documentation Only forbids this skill outright.
